...
echo "=== Dumping database ${DB_NAME} from barman S3 backup ==="
echo "  S3 URL: $S3_URL"
...
podman run --rm \
  ...
  sh -c "echo 'ssl = off' >> /var/lib/postgresql/data/postgresql.conf && \
         ...
         sed -i '/^host.*cert.*map=/s/^/# DISABLED for dump: /' /var/lib/postgresql/data/pg_hba.conf && \
...
mkdir -p "$DUMP_DIR"
DUMP_FILE="${DUMP_DIR}/${DB_NAME}_${BACKUP_ID}.dump"
echo "  Dumping database ${DB_NAME} to ${DUMP_FILE}..."
podman exec -i "$TMP_CONTAINER" pg_dump -U postgres -Fc -d "$DB_NAME" > "$DUMP_FILE"
...
echo ""
echo "  Import with:"
echo "    mise run clean-db-on-local-podman-postgres"
echo "    mise run restore-dump-to-local-podman-postgres ${DUMP_FILE}"
