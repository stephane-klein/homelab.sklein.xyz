#!/usr/bin/env osh
set -e

cd "$(dirname "$0")/"

mkdir -p images/

tmpdir=$(mktemp -d)

... coreos-installer download
    -s stable
    -a x86_64
    -p metal
    -f iso
    -d
    -C "$tmpdir";

mv "$tmpdir"/* images/fedora-coreos-live-iso.x86_64.iso
rmdir "$tmpdir"

export NUC_I3_GEN5_STEPHANE_PASSWORD_HASH="$(echo \"${NUC_I3_GEN5_STEPHANE_PASSWORD}\" | mkpasswd --method=yescrypt -s)"
export NUC_I3_GEN5_NETBIRD_SETUP_KEY="$(tofu output -raw setup_key_nuc_i3_gen5)"

minijinja-cli --env coreos-custom-iso-config.bu.tmpl > coreos-custom-iso-config.bu
butane coreos-custom-iso-config.bu > coreos-custom-iso-config.ign

rm -f images/fedora-coreos-for-nuc-i3-gen5.iso

... coreos-installer iso customize
    --pre-install run-wipefs.sh
    --dest-ignition coreos-custom-iso-config.ign
    --dest-console ttyS0,115200n8
    --dest-console tty0
    --dest-device /dev/sda
    --live-karg-append "vconsole.keymap=fr-bepo locale.LANG=fr_FR.UTF-8"
    -o images/fedora-coreos-for-nuc-i3-gen5.iso
    images/fedora-coreos-live-iso.x86_64.iso;

cat << 'EOF' 
CoreOS custom iso builded in: images/fedora-coreos-for-nuc-i3-gen5.iso

Execute:

$ ./write-to-usb-interactive.sh

to write ISO image on USB key
EOF
