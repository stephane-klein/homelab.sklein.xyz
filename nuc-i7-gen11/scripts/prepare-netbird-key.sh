...
MACHINE_NAME="nuc-i7-gen11.homelab.stephane-klein.info"
...
if grep -q "^NUC_I7_GEN11_NETBIRD_SETUP_KEY=" "$SECRET_FILE"; then
  sed -i "s/^NUC_I7_GEN11_NETBIRD_SETUP_KEY=.*/NUC_I7_GEN11_NETBIRD_SETUP_KEY=\"$SETUP_KEY\"/" "$SECRET_FILE"
else
  echo "NUC_I7_GEN11_NETBIRD_SETUP_KEY=\"$SETUP_KEY\"" >> "$SECRET_FILE"
fi

echo "  NUC_I7_GEN11_NETBIRD_SETUP_KEY written to: $SECRET_FILE"
